...
public class ObjectLoader {
    ...
    private static void processVertex(int pos,int texCoord, int normal,
                                List<Vector2f> texCoordlist, List<Vector3f> normalList, List<Integer> indicesList,
    ...
    public Model loadModel(float[] vertices, float[] TextureCoords,float[] normalArr , int[] indices) {
        int id = createVAO();
        storeIndicesBuffer(indices);
        storeDataInAttribList(0,3,vertices);
        storeDataInAttribList(1,2,TextureCoords);
        storeDataInAttribList(2,3,normalArr);
        unbind();
    ...
    private void storeDataInAttribList(int attribNo, int vertexCount, float[] data) {
        ...
        GL20.glEnableVertexAttribArray(attribNo);   // ← enable it!
